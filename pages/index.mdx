...
```
The `exchangeAndVerifyIdToken` function initiates the authentication process by using the Webflow Designer API’s `getIdToken`  and `getSiteInfo` methods to retrieve the ID token and Site ID from the Designer Extension.
...
Add an endpoint on your server to handle the OAuth callback and retrieve an `authorization_code` from the URI’s query parameters. Then, exchange the code for an accessToken for your user.
...
```
Now that you can retrieve an access token, let’s configure your database to store user credentials and authorization details in `database.js`. This ensures secure and persistent access to user data.
...
Next, you’ll store the site and user access tokens in the database using functions that insert new records if they don’t already exist.
...
```
Finally, you’ll retrieve the stored access tokens using functions designed to fetch access tokens based on site ID or user ID.
...
```javascript jwt.js
```
With our database configured to handle access tokens, it’s time to implement JSON Web Tokens (JWT) to securely manage sessions and authenticate requests in the Data Client. JWT will enable us to issue session tokens and validate them, allowing for a secure, stateless authentication flow.
...
This Session Token, which includes the user’s details, will later be verified for secure access. The token is set to expire after a defined time (in this example, 24 hours), ensuring that the session remains secure and temporary.
...
```
Using the `accessToken`, this endpoint sends an authenticated request to Webflow’s Resolve ID Token endpoint, which validates the `idToken` received from the Designer Extension.
...
```
Finally, the Access Token is stored in the database with the user’s ID using `db.insertUserAuthorization`, enabling easy access and authorization for subsequent requests
...
```
Once authenticated, the middleware retrieves the associated `accessToken` and attaches it to the request. The Data Client then uses this token to initialize the Webflow client and fetch data from Webflow’s Sites endpoint.
