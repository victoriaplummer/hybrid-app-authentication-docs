...
- **[Authorizes sites upon App installation](https://developers.webflow.com/data/reference/oauth-app)** 
- **Transmits a Site ID and ID Token** <br/>
  Send a [Site ID](https://docs.developers.webflow.com/designer/reference/get-site-info) and [ID Token](https://docs.developers.webflow.com/designer/reference/get-user-id-token) from the Designer Extension to the Data Client for verification.
- **Resolves the ID Token on the Server Side**<br/>
  The Data Client resolves the [ID Token](https://docs.developers.webflow.com/data/v2.0.0-beta/reference/resolve-1) to get user details from Webflow and verify access.
- **Maps user authorization**<br/>
  Establish a link between the user and their authorized sites, allowing for secure access to site data.
- **Creates a Session Token for the user**<br/>
...
- **Makes authenticated requests to Webflow’s Data APIs**<br/>
...
1. **Retrieve Site Access Tokens with `getAccessTokenFromSiteId`**<br/>
This function retrieves the access token for a specific `siteId`. We'll use this function to obtain user details from Webflow's [Resolve ID Token endpoint](https://docs.developers.webflow.com/data/v2.0.0-beta/reference/resolve-1) when a Designer Extension sends an `idToken` and `siteId` to our `/token` endpoint.
2. **Retrieve User Access Tokens with `getAccessTokenFromUserId`**<br/>
...
```javascript jwt.js
```
With our database configured to handle access tokens, it’s time to implement [JSON Web Tokens (JWT)](https://jwt.org) to securely manage sessions and authenticate requests in the Data Client. 
...
```
Using the `accessToken`, this endpoint sends an authenticated request to Webflow’s [Resolve ID Token endpoint](https://docs.developers.webflow.com/data/v2.0.0-beta/reference/resolve-1), which validates the `idToken` received from the Designer Extension, and returns the following user details: `id`, `email`, `firstName`, and `lastName`.
...
```
The Data Client then uses the `accessToken` attached to the request to initialize the Webflow client and fetch data from Webflow’s [List Sites endpoint.](https://docs.developers.webflow.com/data/v2.0.0-beta/reference/list-sites-1)
