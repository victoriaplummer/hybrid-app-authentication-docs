...
Learn how to establish a secure connection between your Designer Extension and Data Client. The provided client- and server- side code uses [JSON Web Tokens (JWTs)](http://jwt.io) to make authenticated requests without compromising client credentials.
...
### Prerequisites
- A [Hybrid App](https://docs.developers.webflow.com/data/docs/register-an-app) with the `sites:read` and `authorized_user:read` [scopes](https://docs.developers.webflow.com/data/reference/scopes)
- The `CLIENT_ID` and `CLIENT_SECRET` for your App
- A bearer key generated from your app to use an App token
- An [ngrok](https://ngrok.com/) authentication token
- Basic knowledge of Node.js and Express and familiarity with building React Single-Page Applications
